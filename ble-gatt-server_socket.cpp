...
#include "gatt-server-master/include/Gobbledegook.h"
...
static std::string dataSendBuf = "";
static std::string dataRecvBuf = "";
...
const void *dataGetter(const char *pName)
{
	...
	{
		std::cout << "Getter:" << serverDataTextString.c_str() << "\n";
		return serverDataTextString.c_str();
	}
	else if(strName == "uart_tx")
	{
		std::cout << "send:" << dataSendBuf.c_str() << "\n";
		return dataSendBuf.c_str();
	}
	else if(strName == "uart_rx")
	{
		std::cout << "recv:" << dataRecvBuf.c_str() << "\n";
		
        //.....
        send(linkSock, dataRecvBuf.c_str() , strlen(dataRecvBuf.c_str()), 0);
		//==========echo==========
		//dataSendBuf = dataRecvBuf;
		//ggkNofifyUpdatedCharacteristic("/com/raspberry_ggk_ble/Nordic_UART_Service/UART_TX");
		//========================
		//.....
		return dataRecvBuf.c_str();
	}
...
int dataSetter(const char *pName, const void *pData)
{
	...
	else if (strName == "text/string")
	{
		...
		std::cout << "Setter:" << serverDataTextString << "\n";
		LogDebug((std::string("Server data: text string set to '") + serverDataTextString + "'").c_str());
		return 1;
	}
	else if (strName == "uart_rx")
	{
		dataRecvBuf = static_cast<const char *>(pData);
		
//LogDebug((std::string("Server data: text string set to '") + dataSendBuf + "'").c_str());
		return 1;
	}
...
void socketRecv()
{
	...
	if (bind(sServ, (struct sockaddr *)&servAddr, sizeof(servAddr)) == -1) 
	{	
		close(sServ);   //.....
		return -1;
	}
	...
	{			
		...
		linkSock = accept(sServ, (struct sockaddr *) &cliAddr, (socklen_t *) &cliAddrLen);  //.....
		...
		while (1)
		{
			dataLen = recv(linkSock, buf, BUF_SIZE, 0);	
			if (dataLen <= 0)
			{
				printf("recv error.\n");
				break;
			...
			printf("%s\n", buf);
            //.....
			/*dataLen = send(linkSock, buf, dataLen, 0); 
			if (dataLen <= 0)
			{
				printf("sent error.\n");
				break;
...
int main(int argc, char **ppArgv)
{
	...
	//
	if (!ggkStart("raspberry_ggk_ble", "pi3-ble", "pi3-ble", dataGetter, dataSetter, kMaxAsyncInitTimeoutMS))
	{
		return -1;
	}
	...
	*test = 0;
    ...
    std::thread socketThread(socketRecv);   //.....
